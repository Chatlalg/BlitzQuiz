...
const createToken = (email, userId, role) => {
  return sign({ email, userId, role }, process.env.JWT_KEY, {
    expiresIn: maxAge,
  });
};
...
export const generateQuiz = async (req, res) => {
 ...
 console.log("here is emai;l",email)
  ...
  if (jsonParts.length === 2) {
    ...
    if(uniqueId){
        const update = await quiz.findOneAndUpdate({email:email},{quizquestion:jsonParts[0],quizanswer:jsonParts[1],quizid:uniqueId},      { new: true, upsert: true, runValidators: true }  // options
        )
        console.log(update);
